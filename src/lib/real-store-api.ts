...
class RealStoreAPI {
  ...
  async initializeConnection(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      // Priority order of store URLs to try
      const storeUrls = [
        'https://inkey-list-clone.netlify.app',
        'https://inkey-list-clone.vercel.app',
        process.env.NEXT_PUBLIC_MAIN_STORE_URL,
        'https://api.inkeylist.com', // Official API if available
      ].filter(Boolean);
  ...
  // ... existing code ... <rest of the methods remain the same>
